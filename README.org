...
- [[#usage][Usage]]
  ...
  - [[#markua-document-settings][Markua document settings]]
...
2. If the =#+LEANPUB_BOOK_ID_AS_FILENAME= is set and the heading has a =NAME=, =CUSTOM_ID= or  =ID= property, it is used as the base filename, and used to construct the filename inside =manuscript=.
3. Otherwise, the filename is generated based on the heading title by lowercasing it and replacing all non-alphanumeric characters with hyphens.

Exports do not modify the source Org file. In particular, automatically computed filenames are not written back into the =EXPORT_FILE_NAME= property. If you set the =#+LEANPUB_BOOK_RECOMPUTE_FILENAMES= attribute in your file, the filename is recomputed for export every time and any existing =EXPORT_FILE_NAME= property is ignored. This can be useful to keep the filenames in sync when you change the heading titles in your document, but be aware that files exported earlier with a different name will not be removed automatically.
...
In book export, headings tagged =frontmatter=, =mainmatter= or =backmatter= are used to generate the corresponding [[https://leanpub.com/markua/read#directives][Markua directives]] in the manuscript structure. For Markua 0.30, only =mainmatter= and =backmatter= are emitted; =frontmatter= is kept only for Markua 0.10 compatibility. These tags are not interpreted during plain Markua export.

If a level-1 heading has the =part= tag, it is exported as a part heading. In Markua 0.10 this is rendered as ="# Title #"=. In Markua 0.30 it is rendered as:
...
In [[https://leanpub.com/lfm/read#leanpub-auto-parts][LFM]] it is rendered as =-# Title=.
...
- An element's =#+CAPTION=, if specified, is used for the =caption= attribute in Markua 0.10, the =title= attribute in Markua 0.30, and the =title= attribute in LFM (see [[#block-captions][Block Captions]] for details of how captions are produced in block elements).
...
You can specify a [[https://leanpub.com/read/markua/leanpub-auto-block-elements#leanpub-auto-using-extension-attributes-on-blurbs-to-add-icon-support][custom icon]] for a block using the =:icon= attribute in an =#+ATTR_LEANPUB= line. For example:
...
- =iconname= (optional) is a [[https://leanpub.com/read/markua/leanpub-auto-block-elements#leanpub-auto-using-extension-attributes-on-blurbs-to-add-icon-support][valid icon name]] to use for the block.
...
** Markua document settings

For Markua 0.30 book export, you can specify [[https://markua.com/#document-settings-m-][document settings]] using one or more =#+MARKUA_DOC_SETTINGS= lines. The syntax follows Org argument syntax:

#+begin_src org
 ,#+MARKUA_DOC_SETTINGS: :soft-breaks break :two-space-hack false
 ,#+MARKUA_DOC_SETTINGS: :alt-title title
#+end_src

Only the settings you specify are emitted, so all others keep their Markua defaults. During Markua 0.30 book export, these settings are written to =manuscript/doc_settings.txt= and included automatically at the start of =Book.txt= and =Subset.txt= when present. They are not emitted for Markua 0.10.

The =soft-breaks= setting also affects paragraph rendering. By default, =ox-leanpub-markua= collapses paragraph newlines to spaces. If you specify =:soft-breaks break=, paragraph newlines are preserved as written.

The generated file uses one setting per line, for example:
...
For Markua 0.30, raw HTML export blocks and snippets are ignored, since raw HTML is not supported in that version of the syntax.

** Index entries

Leanpub supports producing indices [[https://community.leanpub.com/t/first-version-of-index-entry-support-is-live-only-for-markua-0-30-books/3859][for books using Markua 0.30]], so =ox-leanpub-markua= exports [[https://orgmode.org/manual/Generating-an-index.html][Org-mode index entries]] using the ={i:...}= syntax used by Markua when =#+MARKUA_VERSION= is set to =0.30=. For Markua 0.10, =#+INDEX= entries are ignored. The value given to =#+INDEX= is passed as-is into the ={i:...}= attributes (including any formatting markup, which needs to be provided in Markua format), with one exception: for =see= and =seealso= entries, the format should be =see=otherentry= or =seealso=otherentry=, and it will be converted to the correct syntax on export. The value given to =#+INDEX= must not be enclosed in quotes, but the value passed to =see= or =seealso= may be enclosed in quotes. Separators such as ~!~ and ~|~ must be escaped with a backslash.
...
| =#+LEANPUB_BOOK_RECOMPUTE_FILENAMES= | =nil=              | If set (regardless of its value), recompute chapter filenames for the current export and ignore any existing =EXPORT_FILE_NAME= properties. The source Org file is not modified. Note that if a chapter title has changed since the last export, it will be exported to a new filename, but the old file will not be deleted automatically. |
...
| =#+MARKUA_DOC_SETTINGS=              | =nil=              | (only for Markua 0.30 book export) Define document settings to be written to =manuscript/doc_settings.txt=. The value uses Org header argument syntax such as =:soft-breaks break :two-space-hack false=. Multiple lines are allowed; later values override earlier ones.                    |
| =#+MARKUA_EXPORT_TYPE=               | "book"           | (only for Markua export) Determines the type of export being done. Valid values are "book" and "course".                                                                                                                                                                                  |
| =#+MARKUA_VERSION=                   | "0.10"           | (only for Markua export) Selects the Markua version to target. Valid values are "0.10" and "0.30". This is used to control output for constructs that differ between Markua versions.                                                                                                     |
